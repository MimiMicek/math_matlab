...
%           tRange: the time interval for the solution (or time grid, where solution is provided)
%           xc: a vector with the centers of the computational grid
%           C0: a vector of initial concentration at xc: C(t=0,xc)
%
% Output:   t: vector of time points
%           C: the solution, a 2D matrix containing densities.
%
    % Example:  Solution of a pure diffusion equation with D = 1 (no left side equation trigger plot):
%               solvePDE(@(t,x,C) 0, @(t,x) 0, @(t,x) 1, [0 10], linspace(0,10,100), exp(-((1:100)-50).^2));
...
%               shading interp
%
% Log: ASCH Oct 29, 2019: fixed typo in example
%========================================================================
